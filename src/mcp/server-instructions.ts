...
- **When you already know the exact symbol or function names, name them directly in the query** — e.g. \`_prepare_context _shadow_compare_payment_method\`. Exact-name lookup runs before full-text search and is always reliable. Describing what you're looking for in prose when you have the name causes semantic drift toward unrelated symbols.
- **There is no \`file_path\` parameter.** To scope a query to a specific file or module, include the file name or path in the query string itself.
